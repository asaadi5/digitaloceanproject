<link id="style" href="{{ asset('dist-front/plugins/bootstrap-5/css/bootstrap.min.css') }}" rel="stylesheet" />
<link href="{{ asset('dist-front/css/style.css') }}" rel="stylesheet" />
<link href="{{ asset('dist-front/css/icons.css') }}" rel="stylesheet"/>
<link href="{{ asset('dist-front/plugins/select2/select2.min.css') }}" rel="stylesheet" />
<link href="{{ asset('dist-front/plugins/pscrollbar/pscrollbar.css') }}" rel="stylesheet" />
<link href="{{ asset('dist-front/plugins/owl-carousel/owl.carousel.css') }}" rel="stylesheet" />
<link rel="stylesheet" href="{{ asset('dist-front/css/iziToast.min.css') }}">
